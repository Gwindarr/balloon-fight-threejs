...
export let bridges = [];
export let steppingStones = [];
export let bouncyMushrooms = [];
export let spiralPlatforms = [];
...
export function initEnvironment() {
    ...
    createBouncyMushrooms();
    createSpiralStaircase(); 
    createHeightMarkers();    
    // Make platforms accessible globally for collision system
    window.environmentPlatforms = [...platforms, ...cloudPlatforms, ...bridges, ...steppingStones, ...bouncyMushrooms, ...spiralPlatforms];
}
...
// Create floating bridges connecting different areas
function createFloatingBridges() {
    // Bridge 1: Connecting two existing platforms
    bridges.push(
        createFloatingBridge(-25, 30, -10, 50, 75, 0, 40, 3)
    );
    
    // Bridge 2: A curved bridge in another area
    bridges.push(
        createCurvedBridge(0, 55, -30, 60, 35, -60, 30, 3)
    );
    
    // Bridge 3: A high altitude bridge
    bridges.push(
        createFloatingBridge(-60, 85, 40, 20, 55, 50, 50, 3)
    );
    
    // Bridge 4: Low altitude for recovery
    bridges.push(
        createFloatingBridge(30, 15, 40, -40, 20, 10, 35, 3)
    );
    
    // Bridge 5: Long distance high bridge
    bridges.push(
        createFloatingBridge(-40, 70, -50, 40, 65, 30, 60, 4)
    );
}

// Create a simple straight bridge between two points
function createFloatingBridge(x1, y1, z1, x2, y2, z2, segments, width) {
    ...
    for (let i = 0; i < segments; i++) {
        // Calculate segment position (slightly offset for gaps)
        const offset = i * segmentLength;
        const gapOffset = 0.2; // small gap between planks
        const segmentPos = new THREE.Vector3().copy(startPoint).addScaledVector(direction, offset + segmentLength/2);
        
        // Create plank
        const plankGeometry = new THREE.BoxGeometry(width, 0.5, segmentLength - gapOffset);
        const plankMaterial = new THREE.MeshPhongMaterial({
            color: 0x8B4513 // Brown wood color
        });
        const plank = new THREE.Mesh(plankGeometry, plankMaterial);
        plank.position.copy(segmentPos);
        
        // Align plank along the bridge direction
        plank.lookAt(new THREE.Vector3().copy(segmentPos).add(direction));
        plank.rotation.x = Math.PI / 2; // Rotate to be flat
        ...
        // Add rope sides
        const ropeLeftGeometry = new THREE.CylinderGeometry(0.1, 0.1, segmentLength - gapOffset, 8);
        const ropeRightGeometry = new THREE.CylinderGeometry(0.1, 0.1, segmentLength - gapOffset, 8);
        const ropeMaterial = new THREE.MeshPhongMaterial({
            color: 0x8B4513,
            shininess: 0
        });
        
        const ropeLeft = new THREE.Mesh(ropeLeftGeometry, ropeMaterial);
        const ropeRight = new THREE.Mesh(ropeRightGeometry, ropeMaterial);
        
        // Position ropes on sides of plank
        ropeLeft.position.copy(segmentPos).add(new THREE.Vector3(-width/2 + 0.2, 0.5, 0));
        ropeRight.position.copy(segmentPos).add(new THREE.Vector3(width/2 - 0.2, 0.5, 0));
        
        // Align ropes
        ropeLeft.lookAt(new THREE.Vector3().copy(segmentPos).add(direction));
        ropeLeft.rotation.x = Math.PI / 2;
        ropeRight.lookAt(new THREE.Vector3().copy(segmentPos).add(direction));
        ropeRight.rotation.x = Math.PI / 2;
        
        bridgeGroup.add(ropeLeft);
        bridgeGroup.add(ropeRight);
        
        // Add vertical supports occasionally
        if (i % 3 === 0 || i === segments - 1) {
            ...
            supportLeft.position.copy(segmentPos).add(new THREE.Vector3(-width/2 + 0.2, -1, 0));
            supportRight.position.copy(segmentPos).add(new THREE.Vector3(width/2 - 0.2, -1, 0));
            
            bridgeGroup.add(supportLeft);
            bridgeGroup.add(supportRight);
        }
    }
    ...
    bridgeGroup.userData = {
        type: 'bridge',
        lastPosition: new THREE.Vector3((x1 + x2) / 2, (y1 + y2) / 2, (z1 + z2) / 2)
    };
...
// Create a curved bridge between two points
function createCurvedBridge(x1, y1, z1, x2, y2, z2, segments, width) {
    const bridgeGroup = new THREE.Group();
    ...
    // Create a control point for the curve (higher than both ends)
    const midPoint = new THREE.Vector3().addVectors(startPoint, endPoint).multiplyScalar(0.5);
    const heightOffset = 10; // How much higher the middle should be
    ...
    for (let i = 0; i < segments; i++) {
        ...
        // Get positions and create a small gap
        const gapOffset = 0.02;
        const pos1 = curve.getPoint(t1 + gapOffset * 0.5);
        const pos2 = curve.getPoint(t2 - gapOffset * 0.5);
        
        // Calculate center point and length
        const segmentCenter = new THREE.Vector3().addVectors(pos1, pos2).multiplyScalar(0.5);
        const segmentLength = pos1.distanceTo(pos2);
        
        // Create plank
        const plankGeometry = new THREE.BoxGeometry(width, 0.5, segmentLength);
        const plankMaterial = new THREE.MeshPhongMaterial({
            color: 0x8B4513 // Brown wood color
        });
        const plank = new THREE.Mesh(plankGeometry, plankMaterial);
        plank.position.copy(segmentCenter);
        
        // Get direction to orient the plank
        const tangent = curve.getTangent(t1 + (t2 - t1) / 2).normalize();
        
        // Create a matrix to orient the plank along the tangent
        const lookAtMatrix = new THREE.Matrix4();
        const up = new THREE.Vector3(0, 1, 0);
        const right = new THREE.Vector3().crossVectors(tangent, up).normalize();
        const correctedUp = new THREE.Vector3().crossVectors(right, tangent).normalize();
        
        lookAtMatrix.makeBasis(right, correctedUp, tangent.multiplyScalar(-1));
        plank.setRotationFromMatrix(lookAtMatrix);
        
        plank.castShadow = true;
        plank.receiveShadow = true;
        bridgeGroup.add(plank);
        
        // Add rope sides with proper orientation
        const ropeGeometry = new THREE.CylinderGeometry(0.1, 0.1, segmentLength, 8);
        const ropeMaterial = new THREE.MeshPhongMaterial({
            color: 0x8B4513,
            shininess: 0
        });
        ...
        const ropeLeft = new THREE.Mesh(ropeGeometry, ropeMaterial);
        ropeLeft.position.copy(segmentCenter);
        ropeLeft.position.add(right.clone().multiplyScalar(-width/2 + 0.2));
        ropeLeft.position.add(correctedUp.clone().multiplyScalar(0.5));
        ropeLeft.setRotationFromMatrix(lookAtMatrix);
        
        // Position on right side
        const ropeRight = new THREE.Mesh(ropeGeometry, ropeMaterial);
        ropeRight.position.copy(segmentCenter);
        ropeRight.position.add(right.clone().multiplyScalar(width/2 - 0.2));
        ropeRight.position.add(correctedUp.clone().multiplyScalar(0.5));
        ropeRight.setRotationFromMatrix(lookAtMatrix);
        ...
        if (i % 3 === 0 || i === segments - 1) {
            ...
            const supportGeometry = new THREE.CylinderGeometry(0.2, 0.2, supportHeight, 8);
            const supportMaterial = new THREE.MeshPhongMaterial({
                color: 0x8B4513
            });
            ...
            const supportLeft = new THREE.Mesh(supportGeometry, supportMaterial);
            supportLeft.position.copy(segmentCenter);
            supportLeft.position.add(right.clone().multiplyScalar(-width/2 + 0.2));
            
            const supportRight = new THREE.Mesh(supportGeometry, supportMaterial);
            supportRight.position.copy(segmentCenter);
            supportRight.position.add(right.clone().multiplyScalar(width/2 - 0.2));
            
            // Align supports vertically (regardless of bridge orientation)
            const supportMatrix = new THREE.Matrix4();
            const supportUp = new THREE.Vector3(0, 1, 0);
            const supportRightDir = new THREE.Vector3(1, 0, 0);
            const supportForward = new THREE.Vector3().crossVectors(supportUp, supportRightDir).normalize();            
            supportMatrix.makeBasis(supportRight, supportUp, supportForward);
            supportLeft.setRotationFromMatrix(supportMatrix);
            supportRight.setRotationFromMatrix(supportMatrix);
            
            // Position support downward from bridge
            supportLeft.position.add(new THREE.Vector3(0, -supportHeight/2, 0));
            supportRight.position.add(new THREE.Vector3(0, -supportHeight/2, 0));
    ...
    bridgeGroup.userData = {
        type: 'bridge',
        lastPosition: new THREE.Vector3((x1 + x2) / 2, (y1 + y2) / 2 + heightOffset/2, (z1 + z2) / 2)
    };
...
// Create stepping stones that provide a path upward
function createSteppingStones() {
    // Create several arrays of stepping stones in different areas
    
    // Spiral stepping stones in center area
    createSpiralSteppingStones(0, 5, 0, 20, 80, 16);
    
    // Vertical line of stepping stones
    createVerticalSteppingStones(-80, 10, 30, 14, 70, 12);
    
    // Random scattered stepping stones in an area
    createScatteredSteppingStones(70, 20, -50, 30, 30, 80, 15);
    
    // Zigzag path of stepping stones
    createZigzagSteppingStones(40, 5, 80, 100, 8);
}

// Create stepping stones in a spiral pattern
function createSpiralSteppingStones(centerX, startY, centerZ, radius, maxHeight, count) {
    const heightStep = (maxHeight - startY) / count;
    
    for (let i = 0; i < count; i++) {
        const angle = (i / count) * Math.PI * 6; // Multiple spiral rotations
        const distance = radius * (1 - i/count * 0.7); // Spiral gets tighter toward top
        
        const x = centerX + Math.cos(angle) * distance;
        const y = startY + i * heightStep;
        const z = centerZ + Math.sin(angle) * distance;
        
        const stone = createSteppingStone(x, y, z, 3 - (i/count) * 1.5); // Stones get smaller toward top
        steppingStones.push(stone);
    }
}

// Create stepping stones in a vertical line
function createVerticalSteppingStones(x, startY, z, count, maxHeight, variation) {
    const heightStep = (maxHeight - startY) / count;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const stoneY = startY + i * heightStep;
        const stoneZ = z + offsetZ;
        
        const stone = createSteppingStone(stoneX, stoneY, stoneZ, 2.5);
        steppingStones.push(stone);
    }
}

// Create randomly scattered stepping stones in a box area
function createScatteredSteppingStones(centerX, startY, centerZ, width, depth, maxHeight, count) {
    const heightStep = (maxHeight - startY) / count;
    
    for (let i = 0; i < count; i++) {
        const x = centerX + (Math.random() - 0.5) * width;
        const y = startY + i * heightStep + (Math.random() - 0.5) * heightStep * 0.5;
        const z = centerZ + (Math.random() - 0.5) * depth;
        
        const stone = createSteppingStone(x, y, z, 2 + Math.random() * 1.5);
        steppingStones.push(stone);
    }
}

// Create stepping stones in a zigzag pattern
function createZigzagSteppingStones(startX, startY, startZ, maxHeight, count) {
    const heightStep = (maxHeight - startY) / count;
    const zigzagWidth = 15;
    
    for (let i = 0; i < count; i++) {
        const zigzag = Math.sin((i / count) * Math.PI * 4) * zigzagWidth;
        
        const x = startX + zigzag;
        const y = startY + i * heightStep;
        const z = startZ + i * 3; // Moving slightly forward with each step
        
        const stone = createSteppingStone(x, y, z, 2.5);
        steppingStones.push(stone);
    }
}

// Create an individual stepping stone
function createSteppingStone(x, y, z, radius) {
    const geometry = new THREE.CylinderGeometry(radius, radius * 1.2, 1, 8);
    ...
    for (let i = 0; i < positions.count; i++) {
        ...
        // Only add noise to sides, not top or bottom faces
        if (vertex.y > -0.45 && vertex.y < 0.45) {
    ...
    // Add some moss on top
    if (Math.random() > 0.5) {
        const mossGeometry = new THREE.CircleGeometry(radius * 0.8, 8);
        const mossMaterial = new THREE.MeshPhongMaterial({
            color: 0x567d46, // Moss green
            side: THREE.DoubleSide
        });
        const moss = new THREE.Mesh(mossGeometry, mossMaterial);
        moss.rotation.x = -Math.PI / 2; // Flat on top
        moss.position.y = 0.51; // Slightly above stone
        stone.add(moss);
    }
...
// Create a spiral staircase of platforms for climbing
function createSpiralStaircase() {
    // Create a spiral staircase in a few locations
    createSpiralStaircasePlatforms(-100, 0, 0, 15, 100, 20);
    createSpiralStaircasePlatforms(100, 0, -70, 12, 80, 16);
}

// Create a spiral staircase of platforms
function createSpiralStaircasePlatforms(centerX, startY, centerZ, radius, maxHeight, count) {
    const heightStep = (maxHeight - startY) / count;
    const angleStep = (Math.PI * 2) / (count / 3); // Make multiple revolutions
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Create platform at this position
        const platform = createSpiralPlatform(x, y, z, angle, 5, 5);
        spiralPlatforms.push(platform);
    }
}

// Create an individual platform for the spiral staircase
function createSpiralPlatform(x, y, z, angle, width, depth) {
    ...
    platform.rotation.y = angle; // Orient along spiral
    
    // Add some details to make it look more interesting
    ...
    const glowGeometry = new THREE.PlaneGeometry(width * 0.8, depth * 0.8);
    ...
    glow.position.y = 0.51; // Just above platform
    platform.add(glow);
...
export function updateEnvironment(deltaTime) {  
    ...
    if (cloudPlatforms && cloudPlatforms.length > 0) {
        ...
        // Update bouncy mushrooms
        if (bouncyMushrooms && bouncyMushrooms.length > 0) {
            for (const mushroom of bouncyMushrooms) {
                const anim = mushroom.userData.animationState;
                ...
                if (anim.bouncing) {
                    ...
                    // Animate the mushroom recovering from compression
                    const currentHeight = stem.scale.y;
                    const targetHeight = 1.0; // Original scale
                    
                    // Gradually restore height
                    stem.scale.y += (targetHeight - currentHeight) * anim.recoverySpeed;
                    
                    // Update cap position to match stem
                    cap.position.y = anim.originalHeight * stem.scale.y;
                    
                    // Also update gills
                    if (mushroom.children.length > 3) {
                        const gills = mushroom.children[3];
                        gills.position.y = anim.originalHeight * stem.scale.y - 0.05;
                    }
                    
                    // Stop animation when close enough to original
                    if (Math.abs(stem.scale.y - targetHeight) < 0.01) {
                        anim.bouncing = false;
                        stem.scale.y = targetHeight;
                    }
                }
            }
        }
...
function checkGeometryForNaN(geometry, name) {
    const positions = geometry.attributes.position;
    for (let i = 0; i < positions.count; i++) {
        ...
        if (isNaN(x) || isNaN(y) || isNaN(z)) {
            console.error(`NaN detected in geometry ${name} at index ${i}:`, { x, y, z });
        }
    }
}
